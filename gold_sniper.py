...
# --- ⚙️ DEFAULT CONFIGURATION (Overridden by Arguments) ---
...
def send_discord_alert(title, message, color_type="INFO"):
    ...
    try:
        requests.post(DISCORD_URL, json={
            "username": f"Sniper {MAGIC_NUMBER}",
            "embeds": [{
                "title": title, "description": message,
                "color": colors.get(color_type, 3447003),
                "footer": {"text": f"Symbol: {SYMBOL} | Magic: {MAGIC_NUMBER}"},
...
if __name__ == "__main__":
    # --- ARGUMENT PARSING FOR MULTI-ACCOUNT SUPPORT ---
    parser = argparse.ArgumentParser()
    parser.add_argument("--path", type=str, required=True, help="Path to terminal64.exe")
    parser.add_argument("--magic", type=int, default=555999, help="Magic Number")
    ...
    if not mt5.initialize(path=args.path):
        print(f"❌ Init Failed for {args.path}")
        print(f"Error: {mt5.last_error()}")
        sys.exit()
    
    # Ensure Symbol is visible
    if not mt5.symbol_select(SYMBOL, True):
        print(f"❌ Symbol {SYMBOL} not found in this terminal!")
        sys.exit()
    ...
    send_discord_alert("🤖 Bot Started", f"Account: {mt5.account_info().login}\nSymbol: {SYMBOL}", "INFO")
